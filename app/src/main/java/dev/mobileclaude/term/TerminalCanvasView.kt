...
class TerminalCanvasView(context: Context) : View(context) {
    ...
    private fun scrollLines(lines: Int, e: MotionEvent) {
        val emu = emulator ?: return
        val up = lines < 0
        val count = abs(lines)
        when {
    ...
    private fun cellAt(e: MotionEvent): Pair<Int, Int> {
        val emu = emulator!!
        val col = ((e.x - padding) / renderer.fontWidth).toInt().coerceIn(0, emu.mColumns - 1) + 1
        val row = ((e.y - padding) / renderer.fontLineSpacing).toInt().coerceIn(0, emu.mRows - 1) + 1
        return col to row
    }
    ...
    fun copyAllText(): String = emulator?.screen?.transcriptTextWithoutJoinedLines ?: ""
